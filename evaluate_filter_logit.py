...
def format_eval(input_fol, gold_input_fol, param, eval_script = None):
	"""
	...
	if not eval_script:
		...
		return None
	output_folder = os.path.join('LogitFilter_eval_test', param)
...
def general_eval(input_dir, param):
	output_folder = os.path.join('LogitFilter_eval_test', param)
...
if __name__== "__main__":
	if not os.path.exists('LogitFilter_eval_test'):
		os.mkdir('LogitFilter_eval_test')
	main_folder = 'Logit_output_filter_DM_test/'
	for param in os.listdir(main_folder):
		input_dir = os.path.join(main_folder, param)
		if not input_dir.endswith('MD'):
			path_to_files_sc = input_dir
			path_to_files_md = input_dir + '_MD'
			format_eval(path_to_files_sc, path_to_gold_ratings_sc, param, 'score_scale.pl') #also can use score_maxdiff.pl
			format_eval(path_to_files_md, path_to_gold_ratings_md, param, 'score_maxdiff.pl') #also can use score_maxdiff.pl		
			general_eval(input_dir, param)
