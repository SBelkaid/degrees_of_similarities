...
def format_eval(input_fol, gold_input_fol, i, eval_script = None):
	"""
	...
	if not eval_script:
		...
		return None
	output_folder = os.path.join('VectorSpaceBackoff_eval_test', i)
...
def iter_eval(input_dir):
	output_folder = os.path.join('VectorSpaceBackoff_eval_test', i)
	values_MD = defaultdict(list)
	values_S = defaultdict(list)
	for f in os.listdir(output_folder):
		if not f.startswith('General_eval_iter_Maxdiff') and not f.startswith('General_eval_iter_Scale'):
			md_file = open(os.path.join(output_folder, 'General_eval_Maxdiff'), 'r')
			s_file = open(os.path.join(output_folder, 'General_eval_Scale'), 'r')
			for line in md_file:
				s = line.split('\t')
				attr = s[0]
				value = s[1].replace('\n', '')                    
			...
			for line in s_file:                  
				...
				value = float(value)
				values_S[attr].append(value)
	md_file = open('VectorSpaceBackoff_eval_test/General_eval_iter_Maxdiff', 'w')
	s_file = open('VectorSpaceBackoff_eval_test/General_eval_iter_Scale', 'w')
	for v in values_MD:
...
if __name__== "__main__":
	if not os.path.exists('VectorSpaceBackoff_eval_test'):
		os.mkdir('VectorSpaceBackoff_eval_test')
	main_folder = 'VectorSpaceBackoff_output_test/'
	for i in os.listdir(main_folder):
		input_dir = os.path.join(main_folder, i)
		if not input_dir.endswith('MD'):
			path_to_files_sc = input_dir
			path_to_files_md = input_dir + '_MD'
			format_eval(path_to_files_sc, path_to_gold_ratings_sc, i, 'score_scale.pl') #also can use score_maxdiff.pl
			format_eval(path_to_files_md, path_to_gold_ratings_md, i, 'score_maxdiff.pl') #also can use score_maxdiff.pl
			general_eval(input_dir, i)
	iter_eval(main_folder)
