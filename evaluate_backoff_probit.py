...
def format_eval(input_fol, gold_input_fol, i, eval_script = None):
	...
	if not eval_script:
		...
		return None
	output_folder = os.path.join('ProbitBackoff_eval', i)
	...
	for f in os.listdir(output_folder):
		if not f.startswith('General_eval_iter_Maxdiff') and not f.startswith('General_eval_iter_Scale'):
			...
			for line in s_file:                  
				...
				value = float(value)
				values_S[attr].append(value)
	md_file = open('ProbitBackoff_eval/General_eval_iter_Maxdiff', 'w')
	s_file = open('ProbitBackoff_eval/General_eval_iter_Scale', 'w')
	for v in values_MD:
...
if __name__== "__main__":
	if not os.path.exists('ProbitBackoff_eval'):
		os.mkdir('ProbitBackoff_eval')
	main_folder = 'Probit_output_backoff_DM_test/'
